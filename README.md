...
- 9개 차량 기능 Tool 시뮬레이션
- 40개 이상의 Capability 정의 · 46개 이상의 통합 시나리오 · 140개 이상의 pytest 기반 백엔드 테스트
...
아래 경로에 스크린샷을 추가하면 README 데모 섹션을 바로 완성할 수 있습니다.

### Agent Console

- 권장 파일: `docs/images/agent-console.png`
- 예시 내용: 발화 입력, Vehicle Cockpit, Agent Flow 타임라인

### Scenario Runner

- 권장 파일: `docs/images/scenario-runner.png`
- 예시 내용: intent / tool / safety pass-fail 검증 화면

### Evaluation Dashboard

- 권장 파일: `docs/images/evaluation-dashboard.png`
- 예시 내용: 성공률, intent 분포, tool 사용량 시각화
...
| **Safety Guard** | 주행 중 위험 제어 차단, 졸음 감지 시 휴식 우선 응답, 필수 slot 누락 시 clarification |
...
| 항목 | 현재 기준 |
|------|-------------------|
| pytest | 140개 이상의 backend tests (`backend` 가상환경에서 `python -m pytest --collect-only -q`로 재확인 가능) |
