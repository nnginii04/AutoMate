...
한국어 차량 내 발화를 이해하고, 차량 기능을 안전하게 실행하는 **인-카 AI Agent MVP**입니다.

운전자의 자연어 요청(예: "나 좀 추워", "집으로 안내해줘", "지금 제한속도 몇이야?")을 받아 **Intent 분류 → Slot 추출 → Safety 검증 → Tool 실행 → 응답 생성**까지 한 번에 처리합니다. React 기반 개발 콘솔에서 Agent 실행, 로그 조회, 평가 지표, 시나리오 테스트를 바로 확인할 수 있습니다.

> 시뮬레이션 전용 프로젝트입니다. 실제 차량 하드웨어를 제어하지 않습니다.
...
| 영역 | 설명 |
|------|------|
| **한국어 NLU** | Capability Catalog 기반 규칙 NLU + (선택) LLM Function Calling 하이브리드 |
| **Vehicle Capability Catalog** | 차량 기능을 데이터로 정의하고 Intent/Tool/Safety를 일관되게 연결 |
| **Tool Registry** | 9개 차량 Tool 등록, 스키마 검증, 실행 |
| **Safety Guard** | 주행 중 위험 제어 차단, 졸음 감지 우선순위, clarification 처리 |
| **Road Context** | 제한속도·과속 여부·도로명·어린이 보호구역 등 주행 도로 정보 응답 |
| **Execution Logs** | 요청별 intent, tool, latency, success/failure 영구 저장 |
| **Scenario Runner** | 46개+ 통합 시나리오로 intent/tool/safety 정확도 자동 검증 |
...
```mermaid
flowchart LR
  User[운전자 발화] --> UI[Frontend Console]
  UI --> API[FastAPI /api/agent/run]
  API --> NLU[Hybrid NLU]
  NLU --> Catalog[Capability Catalog]
  Catalog --> Safety[Safety Guard]
  Safety --> Tools[Tool Registry]
  Tools --> Response[Response Builder]
  Response --> Logs[(SQLite Logs)]
  Response --> UI
```
...
2. `SafetyGuard`가 주행 상태·capability 정책을 검사합니다.
3. `ToolRegistry`가 tool을 실행하고 차량 상태를 갱신합니다.
4. 결과를 `final_response`로 조합하고 execution log에 저장합니다.
...
│   ├── app/
│   │   ├── data/
│   │   │   ├── vehicle-capabilities.json   # 차량 기능 사전 (40+ capabilities)
│   │   │   └── test-scenarios.json         # 통합 테스트 시나리오 (46개)
│   │   ├── nlu/                            # Rule-based + Hybrid + LLM NLU
│   │   ├── safety/                         # Safety Guard
│   │   ├── services/                       # Agent, Capability, Scenario, Evaluation
│   │   ├── tools/                          # Vehicle Tool 구현체
│   │   └── routers/                        # FastAPI 라우터
│   ├── tests/                              # pytest (146 tests)
...
`LLM_ENABLED=false`이거나 API 키가 없으면 **규칙 기반 NLU**만 사용합니다. LLM 실패 시에도 자동으로 rule-based로 fallback합니다.
...
| 졸음 감지 + 음악/내비 요청 | 휴게소 안내 우선 (`requires_clarification`) |
| 비 오는 날 창문 열림 + 창문 열기 요청 | 차단 + 안전 안내 |
| 과속 상태 질의 | 실행 허용, 응답에 감속 안내 |
...
r = ScenarioService(SessionLocal()).run_all()
print(r.summary)
"
```
